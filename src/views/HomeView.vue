...
<template>
  <section id="hero" class="heroSection">
    ...
    <h3>
      ...
      <span v-show="elementPlusI18n !== 'zh-tw'" :style="{ marginLeft: elementPlusI18n !== 'zh-tw' ? '5px' : '0px' }">{{
  ...
  <section id="function" class="function">
    ...
    <div class="function_cards" v-if="isPhoneWidth">
      <Card v-for="article, index in cardArticleI18n" :key="article" :title="$t(`home.function.itemTitle${index + 1}`)"
        :content="$t(`home.function.itemContent${index + 1}`)" :img="isDark ? article.darkImg : article.img">
      </Card>
    </div>
    <Splide v-else />
  </section>
