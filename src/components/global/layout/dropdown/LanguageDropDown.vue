  ...
  <div class="container">
    <div v-show="isPhoneWidth" class="languageDropdown">
      ...
      <div v-show="isOpenOption" class="languageDropdown_option">
        <span v-for="option in languageOptionPhone" @click="chooseLanguage(option.localeCode); toggleOpenMenu();">{{
          option.label }}</span>
      </div>
    </div>

    <el-dropdown v-show="!isPhoneWidth" class="languageDropdown" @command="updateLanguage"
      @visible-change="isClickLangDropdown" trigger="click" :teleported="false">
      <span class="languageDropdown_text">{{ languageLabel }}
        <IconBasic class="languageDropdown_icon" :name="arrowIcon" :key="arrowIcon" :width="16" :height="16" />
      </span>
      <template #dropdown>
        <el-dropdown-menu class="languageDropdown_menu">
          <el-dropdown-item v-for="item in languageOption" :command="item.label" class="languageDropdown_menu_item">{{
            item.dropDownOption }}</el-dropdown-item>
        </el-dropdown-menu>
      </template>
    </el-dropdown>
  </div>
